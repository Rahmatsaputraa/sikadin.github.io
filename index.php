                        ...
                        <div class="col-sm-8 mb-2">
                            ...
                            if(mysqli_num_rows($barang)) {?>
                                <?php while($row= mysqli_fetch_array($barang))
                                 {?>
                                    <option value="<?php echo $row["id_barang"]?>"> <?php echo $row["id_barang"]?>
                                <?php $jsArray .= "harga_barang['" . $row['id_barang'] . "'] = {harga_barang:'" . addslashes($row['harga_barang']) . "'};";
                                $jsArray1 .= "nama_barang['" . $row['id_barang'] . "'] = {nama_barang:'" . addslashes($row['nama_barang']) . "'};"; } ?>
                            <?php } ?>
                                </datalist>
...
function format_ribuan($nilai) {
    if (is_numeric($nilai)) {
        return number_format(floatval($nilai), 0, ',', '.');
    } else {
        return $nilai; // Mengembalikan nilai apa adanya jika bukan angka
    }
            }
            ...
            while ($r = $query->fetch_assoc()) {
                $harga_barang = floatval($r['harga_barang']);
                $quantity = intval($r['quantity']);
                $total_barang = $harga_barang * $quantity;
                $total += $total_barang;
                
                $no++;
            }

            // Mengambil jumlah yang dibayarkan oleh pelanggan (contoh, Anda harus mengganti ini dengan input yang sesuai)
            
            if (isset($_POST['bayar'])) {
                $bayar = floatval($_POST['bayar']);
            } else {
                // Penanganan jika kunci "bayar" tidak ada dalam $_POST
                // Misalnya, berikan nilai default atau tampilkan pesan kesalahan.
                $bayar = 0; // Nilai default
            }
...
?>
           ...
           <form method="POST">
    <div class="form-group row mb-0">
        <input type="hidden" class="form-control" name="no_transaksi" value="<?= $kodeCart ?>" readonly>
        <input type="hidden" class="form-control" value="<?= $subtotal ?>" id="hargatotal" readonly>
...
if (isset($_POST['save1'])) {
    ...
    // Ambil nilai bayar dari $_POST jika tersedia, jika tidak, berikan nilai default 0
    $bayar = isset($_POST['bayar']) ? floatval($_POST['bayar']) : 0;
    $subtotal = $total; // Gantilah ini dengan perhitungan yang sesuai
    $kembalian = $bayar - $subtotal;
              ...
              <div class="col-12" >
                  <hr class="mt-2">
                    ...
                    <li class="list-group-item p-0 border-0 d-flex justify-content-between align-items-center">
                        <b>Total</b>
                        <span><b><?php echo format_ribuan($subtotal); ?></b></span>
                    </li>
                    <li class="list-group-item p-0 border-0 d-flex justify-content-between align-items-center">
                        <b>Bayar</b>
